-- Migracao inicial: clientes, agendamentos, consultas + RLS
...
create policy if not exists clientes_select on public.clientes for select using (user_id = auth.uid());
create policy if not exists clientes_insert on public.clientes for insert with check (user_id = auth.uid());
create policy if not exists clientes_update on public.clientes for update using (user_id = auth.uid());

create policy if not exists agendamentos_select on public.agendamentos for select using (user_id = auth.uid());
create policy if not exists agendamentos_insert on public.agendamentos for insert with check (user_id = auth.uid());
create policy if not exists agendamentos_update on public.agendamentos for update using (user_id = auth.uid());

create policy if not exists consultas_select on public.consultas for select using (
	exists(select 1 from public.agendamentos a where a.id = agendamento_id and a.user_id = auth.uid())
);
create policy if not exists consultas_insert on public.consultas for insert with check (
	exists(select 1 from public.agendamentos a where a.id = agendamento_id and a.user_id = auth.uid())
);
create policy if not exists consultas_update on public.consultas for update using (
	exists(select 1 from public.agendamentos a where a.id = agendamento_id and a.user_id = auth.uid())
);
